...
function updateClock() {
    ...
    const yellowAngle = ((elapsed + 15) % 60) * 6; // Décalée de 15s
    const blackAngle = ((elapsed + 30) % 60) * 6; // Décalée de 30s
    const greenAngle = ((elapsed + 45) % 60) * 6; // Décalée de 45s
    ...
    blackHand.style.transform = `translate(-50%, -100%) rotate(${blackAngle}deg)`;
...
resetButton.addEventListener('click', () => {
    ...
    yellowHand.style.transform = `translate(-50%, -100%) rotate(90deg)`; // 15s -> 90°
    blackHand.style.transform = `translate(-50%, -100%) rotate(180deg)`; // 30s -> 180°
    greenHand.style.transform = `translate(-50%, -100%) rotate(270deg)`; // 45s -> 270°
